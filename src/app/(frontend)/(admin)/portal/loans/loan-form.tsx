    ...
    <form onSubmit={onSubmitHandler}>
      ...
      <div className="grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-2">
        ...
        <div>
          ...
          <Select key={`loan-type-${initialData?.id ?? 'new'}`} defaultValue={data.loanType} options={[
            { value: 'personal', label: 'Personal Loan' }, { value: 'home', label: 'Home Loan' },
            { value: 'car', label: 'Car Loan' }, { value: 'education', label: 'Education Loan' },
            { value: 'business', label: 'Business Loan' }, { value: 'credit-card', label: 'Credit Card' },
            { value: 'other', label: 'Other' },
          ]} placeholder="Select loan type" onChange={(value) => setData({ ...data, loanType: value })} />
        </div>
        <div>
        ...
        <div>
          ...
          <Select key={`interest-type-${initialData?.id ?? 'new'}`} defaultValue={data.interestType} options={[
            { value: 'fixed', label: 'Fixed' }, { value: 'variable', label: 'Variable' },
          ]} placeholder="Select interest type" onChange={(value) => setData({ ...data, interestType: value })} />
        </div>
        <div>
        ...
        <div>
          ...
          <Select key={`payment-frequency-${initialData?.id ?? 'new'}`} defaultValue={data.paymentFrequency} options={[
            { value: 'weekly', label: 'Weekly' }, { value: 'bi-weekly', label: 'Bi-weekly' },
            { value: 'monthly', label: 'Monthly' }, { value: 'quarterly', label: 'Quarterly' },
            { value: 'yearly', label: 'Yearly' },
          ]} placeholder="Select frequency" onChange={(value) => setData({ ...data, paymentFrequency: value })} />
        </div>
        <div>
        ...
        <div>
          <Label>Status</Label>
          <Select key={`loan-status-${initialData?.id ?? 'new'}`} defaultValue={data.status} options={[
            { value: 'active', label: 'Active' }, { value: 'paid-off', label: 'Paid Off' },
            { value: 'overdue', label: 'Overdue' }, { value: 'defaulted', label: 'Defaulted' },
          ]} placeholder="Select status" onChange={(value) => setData({ ...data, status: value })} />
        </div>
      ...
      <div className="mt-6 flex w-full items-center justify-end gap-3">
        ...
        <Button size="sm" type="submit">{mode === 'edit' ? 'Save Changes' : 'Create'}</Button>
      </div>
    </form>
  )
}
