...
const LoanCard = (props: LoanCardProps) => {
  ...
  let remaining = 0
  const percentPaid =
    loan.terms && loan.termsPaid ? `${(loan.termsPaid / loan.terms) * 100}%` : '0%'
  const totalPaid =
  ...
  if (loan.interestType === 'fixed') {
    remaining = loan.principalAmount - totalPaid
  }
    ...
    <div className="w-full rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-12 lg:items-start">
        ...
        <div className="lg:col-span-2">
          ...
          <p className="mt-1 text-lg font-semibold text-gray-600 dark:text-white">
            {formatAmount(amount)}
          </p>
        </div>
        ...
        {/* View details */}
        <div className="flex items-start justify-start lg:col-span-1 lg:justify-end">
          <button
            type="button"
            className="flex items-center gap-1 text-sm font-medium text-gray-700 transition hover:text-primary-600 dark:text-gray-300 dark:hover:text-primary-400"
          >
            View Details
