    ...
    <form onSubmit={onSubmitHandler}>
      ...
      <div className="grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-2">
        ...
        <div>
          ...
          <Select
            key={`bill-category-${initialData?.id ?? 'new'}`}
            defaultValue={data.category ?? ''}
            options={[
              { value: 'electricity', label: 'Electricity' }, { value: 'water', label: 'Water' },
              { value: 'internet', label: 'Internet' }, { value: 'rent', label: 'Rent' },
              { value: 'mobile', label: 'Mobile' }, { value: 'telephone', label: 'Telephone' },
              { value: 'insurance', label: 'Insurance' }, { value: 'credit-card', label: 'Credit Card' },
              { value: 'loan', label: 'Loan' }, { value: 'government', label: 'Government' },
        ...
        <div>
          ...
          <Select
            key={`bill-type-${initialData?.id ?? 'new'}`}
            defaultValue={data.type ?? ''}
            options={[{ value: 'subscription', label: 'Subscription' }, { value: 'variable', label: 'Variable' }]}
            placeholder="Select bill type"
            onChange={(value) => setData({ ...data, type: value, ...(value === 'variable' ? { amount: 0 } : {}) })}
          />
        </div>
      ...
      <div className="mt-6 flex w-full items-center justify-end gap-3">
        ...
        <Button size="sm" disabled={isSaving}>{isSaving ? 'Saving...' : mode === 'edit' ? 'Save Changes' : 'Create'}</Button>
      </div>
    </form>
  )
}
