...
import { ArrowDownLeft, CircleDollarSign, X } from 'lucide-react'
import { useEffect, useState } from 'react'
import { createPortal } from 'react-dom'
import { dateToReadable, formatAmount } from '@/helper/common.helper'
import type { Transaction } from '@/payload-types'

type DashboardIncomeCardProps = {
  total: number
  periodLabel: string
...
export default function DashboardIncomeCard({ total, periodLabel, transactions }: DashboardIncomeCardProps) {
    ...
    ? createPortal(
        ...
        >
          <div
            role="dialog"
            aria-modal="true"
            aria-labelledby="income-transactions-title"
          ...
          >
            <div className="flex items-start justify-between gap-4 border-b border-gray-100 px-5 py-4 dark:border-gray-800 sm:px-6">
              <div>
                <h2 id="income-transactions-title" className="text-lg font-semibold text-gray-800 dark:text-white/90">
                  Income Transactions
                </h2>
            ...
            <div className="flex items-center justify-between gap-4 border-b border-gray-100 bg-gray-50 px-5 py-3 dark:border-gray-800 dark:bg-white/[0.02] sm:px-6">
              ...
              <span className="text-base font-semibold text-success-600 dark:text-success-400">{formatAmount(total)}</span>
            </div>
              ...
              {transactions.length ? (
                <div className="divide-y divide-gray-100 dark:divide-gray-800">
                  {transactions.map((transaction) => {
                    const source = relationshipName(transaction.account)

                    return (
                      <div key={transaction.id} className="grid grid-cols-[1fr_auto] items-center gap-4 px-5 py-4 sm:px-6">
                        <div className="flex min-w-0 items-center gap-3">
                          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-success-50 text-success-600 ring-1 ring-success-100 dark:bg-success-500/10 dark:text-success-400 dark:ring-success-500/20">
                            <CircleDollarSign className="h-5 w-5" />
                          </div>
                          <div className="min-w-0">
                            <p className="truncate text-sm font-medium capitalize text-gray-800 dark:text-white/90">
                              {transaction.category?.replaceAll('-', ' ') ?? 'Income'}
                            </p>
                            <p className="mt-1 truncate text-xs text-gray-500 dark:text-gray-400">
                              {transaction.notes || source || transaction.source || 'Income'} · {dateToReadable(transaction.date)}
                            </p>
                ...
                <div className="px-5 py-12 text-center text-sm text-gray-500 dark:text-gray-400 sm:px-6">
                  No income transactions for {periodLabel}.
                </div>
              )}
            </div>
          </div>
  ...
  return (
    <>
      <button
        ...
        className="w-full rounded-2xl border border-gray-200 bg-white p-5 text-left transition-all duration-200 hover:-translate-y-0.5 hover:border-success-200 hover:shadow-sm focus:outline-none focus:ring-3 focus:ring-success-500/10 dark:border-gray-800 dark:bg-white/[0.03] dark:hover:border-success-500/30"
        aria-haspopup="dialog"
      >
        <div className="flex items-center justify-between gap-3">
          <p className="text-sm font-medium text-gray-600 dark:text-gray-300">Total Income</p>
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-success-100 text-success-600 dark:bg-success-500/15 dark:text-success-400">
            <ArrowDownLeft className="h-5 w-5" />
          </div>
        </div>
        <p className="mt-4 truncate text-xl font-bold text-success-600 dark:text-success-400">{formatAmount(total)}</p>
        <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Income recorded in {periodLabel} · Click to view</p>
      </button>
